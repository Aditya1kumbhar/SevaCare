...
const getStatusOptions = (t: any) => [
  { value: 'good', label: t.optimal, active: 'bg-emerald-50 text-emerald-700 border-emerald-200 shadow-sm' },
  { value: 'fair', label: t.fair, active: 'bg-amber-50 text-amber-700 border-amber-200 shadow-sm' },
  { value: 'poor', label: t.poor, active: 'bg-orange-50 text-orange-700 border-orange-200 shadow-sm' },
  { value: 'critical', label: t.critical, active: 'bg-rose-50 text-rose-700 border-rose-200 shadow-sm' },
]

const getMoodOptions = (t: any) => [
  { value: 'happy', label: t.stable, icon: Smile },
  { value: 'neutral', label: t.neutral, icon: Meh },
  { value: 'sad', label: t.withdrawn, icon: Frown },
  { value: 'agitated', label: t.agitated, icon: Angry },
]
...
export default function LogStatusPage({ params }: { params: Promise<{ residentId: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!status) {
      toast.error(t.pleaseSelectHealthStatus)
      return
    }
    setLoading(true)
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(result.error || t.failedToLogStatus)
        setLoading(false)
        return
      }

      if ((res as any).offline) {
        toast.info(t.savedLocallySyncOnline, {
          icon: '☁️',
        })
      } else {
        toast.success(t.statusLogged)
      }
  ...
  if (!resident) {
    return <p className="text-slate-500 text-center py-16 text-sm font-medium"><Translate id="retrievingRecord" fallback="Retrieving record..." /></p>
  }
    ...
    <div className="space-y-6 max-w-2xl mx-auto bg-slate-50 min-h-screen pb-12">
      <div className="border-b border-slate-200 pb-4 mb-6 pt-4">
        ...
        <p className="text-slate-500 text-sm mt-1">
          <Translate id="patient" fallback="Patient" />: {resident.name} • <Translate id="age" fallback="Age" /> {resident.age}
          {resident.room_number && ` • ${t.rm} ${resident.room_number}`}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4 bg-white border border-slate-200 shadow-sm rounded-2xl p-6">
          <label className="text-sm font-semibold text-slate-900 border-b border-slate-100 pb-3 block">
            <Translate id="overallClinicalStatus" fallback="Overall Clinical Status *" />
          </label>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            ...
            <Translate id="meals" fallback="Meals" /> {mealTaken ? <Translate id="verified" fallback="Verified" /> : <Translate id="pending" fallback="Pending" />}
          </button>
          <button
            type="button"
          ...
          >
            ...
            <Translate id="meds" fallback="Meds" /> {medicationTaken ? <Translate id="verified" fallback="Verified" /> : <Translate id="pending" fallback="Pending" />}
          </button>
        </div>
        ...
        <div className="space-y-4 bg-white border border-slate-200 shadow-sm rounded-2xl p-6">
          <label className="text-sm font-semibold text-slate-900 border-b border-slate-100 pb-3 block">
            <Translate id="behavioralState" fallback="Behavioral State" />
          </label>
        ...
        {/* Notes */}
        <textarea
          placeholder={t.additionalClinicalNotes}
          value={notes}
        ...
        <div className="flex gap-3 sticky bottom-4 p-4 border border-slate-200 shadow-lg rounded-2xl bg-white/95 backdrop-blur-sm">
          ...
          >
            <Translate id="cancel" fallback="Cancel" />
          </button>
          <button
            type="submit"
          ...
          >
            {loading ? t.processing : <><ClipboardList className="w-4 h-4"/> <Translate id="saveEntry" fallback="Save Entry" /></>}
          </button>
        </div>
      </form>
    </div>
